...
def ss_station_bar_menu(q):
    ss_station_bar = ui.form_card(
        box = ui.box('ss_sidebar'),
        items=[
            ui.dropdown(name='ss_station', label='Station (Single)', value=q.client.ss_station, required=True, choices=[ui.choice(str(stn), str(stn)) for stn in stations['TN']], trigger=True)
        ])
...
def plot_ss_cols(q):
    tb = df.loc[('TN', q.client.ss_station), ['Date'] + q.client.ss_col]
...
def station_bar_menu(q):
    station_bar = ui.form_card(
        box = ui.box('sidebar'),
        items=[
            ui.dropdown(name='station', label='Station (Multiple)', values=q.client.station, required=True, choices=[ui.choice(str(stn), str(stn)) for stn in stations['TN']], trigger=True)
        ])
    return station_bar


def plot_aqi(q):
    tb = df.loc[('TN', list(q.client.station)), ['Date', 'AQI']]
    tb.reset_index(inplace=True)
